...
gulp.task('images', function() {
    return gulp.src('app/images/**/*.+(png|jpg|jpeg|gif|svg)')
        ...
        .pipe(gulpIf(isWordpress, gulp.dest(wordpressLocalAssetsPath + '/images')));
});
...
gulp.task('copytheme', function() {
    return gulp.src(wordpressLocalThemePath + "/**/*.*")
        .pipe(gulp.dest(localWordPressThemeBackupFolder));
})

// Copying the PHP customizer file
gulp.task('copycustomizer', function(done) {
    if (fs.existsSync(wordpressLocalThemePath + "/inc/" + wpCustomizerFileName)) {
        console.log('\x1b[33mThe file ' + wordpressLocalThemePath + '/inc/' + wpCustomizerFileName + ' already exists\x1b[0m');
        done();
    } else {
        return gulp.src(customizerFile)
...
gulp.task('includecustomizer', function(cb) {
    fs.appendFile(wordpressLocalThemePath + "/functions.php", '\r\n\r\n\r\n/**\r\n *Theme customizer\r\n */ \r\n require get_stylesheet_directory() . "/inc/stegaru-customizer.php";', cb);
})
...
gulp.task('clean', function() {
    return del([assetsFolder + '/**/*', '!' + assetsFolder + '/images', '!' + assetsFolder + '/images/**/*']);
});
